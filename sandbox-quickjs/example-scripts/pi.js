...
function main(args) {
    ...
    if (args.length < 1) {
        print("usage: Set number of digits");
        return;
    }
    n_digits = args[0] | 0;
    ...
    out = r.toString();
    const res = out[0] + "." + out.slice(1, args[0]+1);
    return res;
}
...
// Example input
const digits = [(typeof input === 'undefined') ? 1000 : input];
// Call the function with example input
console.log(`PI computation for ${digits} digits:`);
const time = Date.now();
const result = main(digits);
console.log(`Calculated digits: ${result.length-2}, last digits: ${result.slice(-5)}`);
